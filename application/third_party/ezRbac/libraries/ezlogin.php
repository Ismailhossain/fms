...
class ezlogin
{
        ...
        if (!is_null($user = $this->CI->ezuser->get_user_by_email($useemail))) {	//email ok
            ...
            if ($this->CI->encrypt->sha1($password.$user->{$this->_user_schema['salt']})===$user->{$this->_user_schema['password']}){		// password ok
                ...
                $this->CI->session->set_userdata(array(
                    'user_id'	=> $user->{$this->_user_schema['employee_id']},
                ...
                // success
                if ($remember) {
                    $this->create_autologin($user->{$this->_user_schema['employee_id']});
                }
                $this->CI->ezuser->update_login_info($user->{$this->_user_schema['employee_id']});
                return TRUE;
            } 														// fail - wrong password
    ...
    private function process_recovery($user){
        $key=$this->CI->ezuser->requestPassword($user->{$this->_user_schema['employee_id']});
